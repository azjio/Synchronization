...
;- GUI
If OpenWindow(#Window, 0, 0, ww, wh, "Синхронизация каталогов", #PB_Window_SystemMenu | #PB_Window_SizeGadget | #PB_Window_MaximizeGadget | #PB_Window_MinimizeGadget | #PB_Window_ScreenCentered)
	CompilerIf  #PB_Compiler_OS = #PB_OS_Linux
	...
	GadgetToolTip(#Open1, "Выбрать папку")
	StringGadget(#StrG1, 10, 7, ww2 - 63, 30, "")
	hListView1 = ListIconGadget(#LV1,10, 40, ww2 - 20, wh - 115, "Путь 1", 400, #PB_ListIcon_CheckBoxes | #PB_ListIcon_FullRowSelect | #PB_ListIcon_AlwaysShowSelection)
	AddGadgetColumn(#LV1, 1, "", 70)
	AddGadgetColumn(#LV1, 2, "", 110)
	ButtonImageGadget(#Copy1, ww2 - 48, wh - 73, 30, 30, ImageID(1))
	GadgetToolTip(#Copy1, "Копировать выбранные в противоположный каталог")
	ButtonImageGadget(#Del1, ww2 - 88, wh - 73, 30, 30, ImageID(3))
	GadgetToolTip(#Del1, "Удалить выбранные")
	CompilerSelect #PB_Compiler_OS
		CompilerCase #PB_OS_Windows
	...
	CompilerEndSelect
	GadgetToolTip(#ChAll1, "Выделить все")

	StringGadget(#StrMask, 10, wh - 73, (ww2 - 130) / 2, 30, "")
	CheckBoxGadget(#ChTMask, (ww2 - 130) / 2 + 15, wh - 73, (ww2 - 130) / 2, 22, "Все файлы", #PB_CheckBox_ThreeState)
	SetGadgetData(#ChTMask, 1)
	CloseGadgetList()

	ContainerGadget(#Container2, ww2, 0, ww2, wh - 39)
	ButtonImageGadget(#Open2, ww2 - 49, 7, 39, 31, ImageID(6))
	GadgetToolTip(#Open2, "Выбрать папку")
	StringGadget(#StrG2, 10, 7, ww2 - 63, 30, "")
	hListView2 = ListIconGadget(#LV2,10, 40, ww2 - 20, wh - 115, "Путь 2", 400, #PB_ListIcon_CheckBoxes | #PB_ListIcon_FullRowSelect | #PB_ListIcon_AlwaysShowSelection)
	AddGadgetColumn(#LV2, 1, "", 70)
	AddGadgetColumn(#LV2, 2, "", 110)
	ButtonImageGadget(#Copy2, 10, wh - 73, 30, 30, ImageID(2))
	GadgetToolTip(#Copy2, "Копировать выбранные в противоположный каталог")
	ButtonImageGadget(#Del2, 50, wh - 73, 30, 30, ImageID(3))
	GadgetToolTip(#Del2, "Удалить выбранные")
	CheckBoxGadget(#ChAll2, 90, wh - 73, 22, 22, "")
	GadgetToolTip(#ChAll2, "Выделить все")
	ButtonGadget(#btnMenuSel, ww2 - 56, wh - 73, 30, 30, Chr($25BC))

; 	CompilerIf  #PB_Compiler_OS = #PB_OS_Linux
	CompilerIf  #PB_Compiler_OS = #PB_OS_Linux
		CheckBoxGadget(#ChFind, 120, wh - 73, 55, 22, "find")
		GadgetToolTip(#ChFind, "Использовать пакет find")
		ChFind = 1
	...
	GadgetToolTip(#Scan, "Найти файлы и сравнить")
	ButtonImageGadget(#Empty, ww - 120, wh - 38, 30, 30, ImageID(5))
	GadgetToolTip(#Empty, "Найти пустые каталоги")
	...
	If CreatePopupMenu(#MenuCont) ; Создаёт всплывающее меню
		MenuItem(0, "Открыть папку файла")
		MenuItem(1, "Открыть файл")
		If isCompareProg
			MenuItem(4, "Сравнить парный")
			MenuItem(5, "Сравнить выделенные")
		EndIf
		MenuItem(6, "Список в буфер обмена")
; 		MenuItem(2, "")
	EndIf


	If CreatePopupMenu(#MenuSel)
		MenuItem(2, "Открыть ini-файл")
		MenuItem(3, "Добавить в ini-файл")
		MenuBar()
...
; GUI2
; If OpenWindow(#WinSet, 0, 0, 340, 210, "Синхронизация каталогов", #PB_Window_Tool | #PB_Window_Invisible | #PB_Window_ScreenCentered)
; 	CompilerIf  #PB_Compiler_OS = #PB_OS_Linux
				...
				Case #ChTMask
					...
					Select itmp
						Case 1
							...
							g_MaskType = 0
							SetGadgetText(#ChTMask, "Все файлы")
						Case 2
							SetGadgetState(#ChTMask, #PB_Checkbox_Checked)
							g_MaskType = 1
							SetGadgetText(#ChTMask, "Маска ext1,ext2")
						Case 3
							SetGadgetState(#ChTMask, #PB_Checkbox_Inbetween)
							g_MaskType = -1
							SetGadgetText(#ChTMask, "Кроме ext1,ext2")
					EndSelect
...
; 						EvGad = GetActiveGadget()
					...
					Case #ChAll2
						ChAll(#LV2, #ChAll2)
					Case #Open1
						tmp$ = PathRequester("Выбор папки", GetHomeDirectory())
						If tmp$
							SetGadgetText(#StrG1, tmp$)
						EndIf
					Case #Open2
...
Procedure AddedSample()
	...
	If isINI
		Name$ = InputRequester("Имя пункта", "", "")
		If Asc(Name$)
...
; 						Debug GroupsMap()
; 						If GroupsMap() > 1
; 							MessageRequester("Ошибка", "Повторяется группа: " + MapKey(GroupsMap()))
; 						EndIf
; 					Next
...
; 		MessageRequester("1", Chr(34) + Path1$ + Chr(34) + " " + Chr(34) + Path2$ + Chr(34))
	Else
		MessageRequester("", "Парного в соседней папке не существует")
	EndIf
EndProcedure
...
; 					удаление пустых папок
	...
	If Asc(Error$)
		MessageRequester("Список ошибок операции", Error$)
	EndIf
EndProcedure
...
; 		пути пусты
		MessageRequester("Ошибка", "Необходимо указать пути.")
		ProcedureReturn
	EndIf
	If Path1$ = Path2$
; 		пути одинаковы
		MessageRequester("Ошибка", "Пути слева и справа одинаковы.")
		ProcedureReturn
	EndIf
	If FileSize(Path1$) <> -2
; 		путь не существует
		MessageRequester("Ошибка", "Путь указан не верно" + " (1).")
		ProcedureReturn
	EndIf
	If FileSize(Path2$) <> -2
		; 		путь не существует
		MessageRequester("Ошибка", "Путь указан не верно" + " (2).")
		ProcedureReturn
	EndIf
	...
	ElseIf Len1 < Len2 And Left(Path2$, Len1 + 1) = Path1$ + #PS$
		FindSub = 1
	EndIf
	If FindSub And MessageRequester("Продолжить?", "Одна папка является подпапкой другой, продолжить?", #PB_MessageRequester_YesNo) = #PB_MessageRequester_No
		ProcedureReturn
	EndIf
...
CompilerEndIf
	...
	SetGadgetText(#StatusBar, "Выполнено за " + FormTime(StartTime) + ", пунктов: " + CountGadgetItems(#LV1) + ":" + CountGadgetItems(#LV2))
EndProcedure
